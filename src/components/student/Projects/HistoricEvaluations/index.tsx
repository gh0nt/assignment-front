...
import { loadProjectsByStudent } from "@/api/project/loadProjectById";
...
export default function HistoricEvaluations() {
  ...
  const getProjects = async () => {
    try {
      ...
      const responseLoadProject = await loadProjectsByStudent(idUser);
      const responseData = await responseLoadProject.data;
      setListProjectsStudent(responseData.$values);
      setIsLoading(false);
    } catch (error: unknown) {
      setMessageErrorLoadProjects(
  ...
  return (
    <>
      <NewProject getProjects={getProjects} />
      <div className="tw-mt-6">
        {isLoading ? (
          ...
          <p className="tw-flex tw-items-center tw-gap-2 tw-text-md">
            ...
            No ha enviado ningún proyecto para evaluar
          </p>
        )}
      </div>
    </>
